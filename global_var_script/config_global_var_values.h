...
//в данном перечислении записываютс€ все переменные, дл€ которых разрешена «јѕ»—№, последний элемент определ€ет общее количество.
typedef enum {
	kTemperature = 0,
...
extern const globalVarModbusAccess global_var_modbus_access;
extern uint32_t* cur_var_value[kVarNum];
